...
const nameClassFunc: agGrid.CellClassFunc = params => {
  return params.data?.[params.data.length - 1] === false ? 'not-qualified' : undefined;
};
...
for (let i = processedData.rounds.length - 1; i >= 0; i--) {
  ...
  columns.push({
    headerName: `${round.label} • ${formatNumber(round.registeredTeams)} registered crews • ${formatNumber(round.rankedTeams)} ranked crews • ${formatNumber(round.qualifiedCrews)} qualified crews`,
    children,
  });
}
